...
public class TwoFoldXValOOCFuplaReader {
	...
	public static int[] m_Order = null;
	public static int[][] m_Parents = null;

	public static int m_BestK_ = -1;
	public static int m_BestattIt = -1;
	...
	public static void main(String[] args) throws Exception {
		...
		datasetName = data.substring(data.lastIndexOf("/")+1, data.length()-5);
		...
		for (int exp = 0; exp < m_nExp; exp++) {
			...
			fuplaOOCFP learner = new fuplaOOCFP();

			String filename = m_DirName + "_" + datasetName + "_Iter" + exp + "_Fold0_" + "k" + m_KDB;
			readFromFile(filename);

			learner.setM_Order(m_Order);
			learner.setM_Parents(m_Parents);
			learner.setM_MVerb(m_MVerb);

			learner.setSelectedK(m_BestK_);
			learner.setSelectedAttributes(m_BestattIt);
			...
			// Train on Fold 1
			// ---------------------------------------------------------
			learner = new fuplaOOCFP();

			filename = m_DirName + "_" + datasetName + "_Iter" + exp + "_Fold1_" + "k" + m_KDB;
			readFromFile(filename);
	...
	private static void readFromFile(String filename) throws NumberFormatException, IOException {
		BufferedReader br = new BufferedReader(new FileReader(filename));
		String line;
		while ((line = br.readLine()) != null) {
			if (line.indexOf('K') != -1) {
				String substr = line.substring(line.indexOf('[')+1, line.indexOf(']'));
				m_BestK_ = Integer.parseInt(substr);
			} 

			if (line.indexOf('n') != -1) {
				String substr = line.substring(line.indexOf('[')+1, line.indexOf(']'));
				m_BestattIt = Integer.parseInt(substr);
			}

			if (line.indexOf("Order") != -1) {
				String order = line.substring(line.indexOf('[')+1, line.indexOf(']'));

				int[] numbers = Arrays.asList(order.split(",")).stream().map(String::trim).mapToInt(Integer::parseInt).toArray();

				ArrayList<Integer> list = new ArrayList();
				for (int i = 0; i < numbers.length; i++) {
					list.add(numbers[i]);
				}
				m_Order = new int[list.size()];
				m_Parents = new int[list.size()][];
				for (int i = 0; i < list.size(); i++) {
					m_Order[i] = list.get(i);
				}

			}

			if (line.indexOf("Att") != -1) {
				String parents = line.substring(line.indexOf('[')+1, line.indexOf(']'));
				int att =  Integer.parseInt(line.substring(line.indexOf("Att") + 3, line.indexOf(':')));

				if (parents.isEmpty()) {

				} else {
					int[] numbers = Arrays.asList(parents.split(",")).stream().map(String::trim).mapToInt(Integer::parseInt).toArray();
					
					ArrayList<Integer> list = new ArrayList();
					for (int i = 0; i < numbers.length; i++) {
							list.add(numbers[i]);
					}
					m_Parents[att] = new int[list.size()];
					for (int i = 0; i < list.size(); i++) {
						m_Parents[att][i] = list.get(i);
					}
	...
	private static void readFromFile(fuplaOOC learner, FileWriter writer) throws IOException {

		writer.write("K:" + learner.getSelectedK() + "\n");
		writer.write("n:" + learner.getSelectedAttributes() + "\n");
		int[] order = learner.getM_Order();
		int[][] parents = learner.getM_Parents();
		...
		for (int i = 0; i < order.length; i++) {
			...
			if (parents[i] != null) {
				for (int j = 0; j < parents[i].length; j++) {
					if (j == parents[i].length - 1)
						writer.write(parents[i][j] + "");
					else
						writer.write(parents[i][j] + ",");
				}
				writer.write("\n");
