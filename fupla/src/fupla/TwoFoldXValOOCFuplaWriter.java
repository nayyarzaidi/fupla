...
public class TwoFoldXValOOCFuplaWriter {
	...
	private static boolean m_MWriteToFile = false;        // -W

	private static Instances instances = null;
	private static int m_nExp = 2;
	...
	public static void main(String[] args) throws Exception {
		...
		for (int exp = 0; exp < m_nExp; exp++) {
			...
			if (m_MWriteToFile) { 
				out = new File(m_DirName + "_" + datasetName + "_Iter" + exp + "_Fold0_" + "k" + m_KDB);
				out.createNewFile();

				writer = new FileWriter(out); 
				
				writeToFile(learner, writer);
			}
			...
			if (m_MWriteToFile) { 
				out = new File(m_DirName + "_" + datasetName + "_Iter" + exp + "_Fold1_" + "k" + m_KDB);
				out.createNewFile();
	...
	private static void writeToFile(fuplaOOC learner, FileWriter writer) throws IOException {
	...
	public static void setOptions(String[] options) throws Exception {
		...
		m_MWriteToFile = Utils.getFlag('W', options);
